"""
Narrative Analysis CLI Module

Clinical narrative analysis with bias pattern assessment for MADEX metric
using extended scenarios with 50 data points per scenario.
"""
...
class NarrativeAnalysis(ClinicalAnalysis, StatisticalAnalysis):
    """Comprehensive narrative analysis using extended 50-point scenarios"""

    def __init__(self, output_dir: Optional[Path] = None):
        """Initialize extended narrative analysis"""
        self.output_dir = output_dir if output_dir else Path("extended_narrative")
        self.output_dir.mkdir(parents=True, exist_ok=True)

        super().__init__("extended_narrative", self.output_dir)
    ...
    def _write_markdown_output(self):
        ...
        # Write file
        output_path = self.output_dir / "narrative_results.md"
...
def run_narrative_analysis(output_dir: Optional[Path] = None):
    """Run clinical narrative analysis."""
    print("=" * 80)
    print("EXTENDED CLINICAL NARRATIVE ANALYSIS")
    print("Using 50 data points per scenario with controlled model bias patterns")
    print("=" * 80)

    analysis = NarrativeAnalysis(output_dir)
