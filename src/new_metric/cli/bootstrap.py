...
class BootstrapAnalysis(ClinicalAnalysis, StatisticalAnalysis):
    ...
    def __init__(
        ...
        self.output_dir = output_dir if output_dir else Path("extended_bootstrap")
        self.output_dir.mkdir(parents=True, exist_ok=True)

        super().__init__("extended_bootstrap", self.output_dir)
    ...
    def _execute_scenario_analysis(self):
        ...
        self.logger.log_narrative(
            f"Bootstrap parameters: n_bootstrap={self.n_bootstrap}, confidence_interval={self.confidence}%"
        )
        ...
        self.markdown_output.extend(
            [
                "## Bootstrap Results",
                "",
                f"### MADEX Confidence Intervals ({self.confidence}%)",
                "",
    ...
    def _analyze_single_scenario(self, scenario_id, step, total):
        ...
        # Bootstrap analysis
        self.logger.log_narrative(
            f"Computing bootstrap confidence intervals (n={self.n_bootstrap})..."
        )
    ...
    def _write_markdown_output(self):
        ...
        self.markdown_output.extend(
            [
                ...
                "### Key Findings",
                "",
                f"1. **Bootstrap Validation**: {self.n_bootstrap} bootstrap samples provide robust confidence intervals for MADEX",
        ...
        # Write file
        output_path = self.output_dir / "bootstrap_results.md"
...
def run_bootstrap_analysis(
    output_dir: Optional[Path] = None, n_bootstrap: int = 1000, confidence: int = 95
):
    ...
    analysis = BootstrapAnalysis(output_dir, n_bootstrap, confidence)
    results = analysis.run_analysis()

    print("\n" + "=" * 80)
    print(
        "Bootstrap analysis complete - check bootstrap_results.md for detailed results"
    )
    print("=" * 80)
