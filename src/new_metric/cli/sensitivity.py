"""
Sensitivity Analysis CLI Module
...
class SensitivityAnalysis(ClinicalAnalysis, StatisticalAnalysis):
    ...
    def __init__(
        ...
        self.output_dir = output_dir if output_dir else Path("extended_sensitivity")
        self.output_dir.mkdir(parents=True, exist_ok=True)

        super().__init__("extended_sensitivity", self.output_dir)
        ...
        if a_range:
            self.a_values = list(range(a_range[0], a_range[1] + 1, 15))
            if a_range[1] not in self.a_values:
                self.a_values.append(a_range[1])
        else:
            self.a_values = [110, 125, 140]

        if b_range:
            self.b_values = list(range(b_range[0], b_range[1] + 1, 15))
            if b_range[1] not in self.b_values:
                self.b_values.append(b_range[1])
        else:
            self.b_values = [40, 55, 70, 80]

        self.c_values = [20, 30, 40, 50, 60, 80]
    ...
    def run_analysis(self):
        ...
        self.markdown_output.extend(
            [
                ...
                f"- **Parameter 'b'** (critical range): {self.b_values}",
                f"- **Parameter 'c'** (slope modifier): {self.c_values}",
                f"- **Total combinations**: {len(self.a_values) * len(self.b_values) * len(self.c_values)}",
                "",
                "---",
                "",
            ]
    ...
    def conduct_sensitivity_analysis(self):
        ...
        self.logger.log_narrative(f"- Parameter 'b' (critical range): {self.b_values}")
        self.logger.log_narrative(f"- Parameter 'c' (slope modifier): {self.c_values}")

        total_combinations = (
            len(self.a_values) * len(self.b_values) * len(self.c_values)
        )
        self.logger.log_narrative(
        ...
        for a, b, c in product(self.a_values, self.b_values, self.c_values):
            current_combo += 1
    ...
    def compare_with_traditional_metrics(self):
        ...
        for scenario_id in self.scenarios.get_scenario_ids():
            ...
            if scenario.model_predictions:
                ...
                self.logger.log_narrative(
                    f"Comparison Metrics: RMSE={metrics_a['RMSE']:.2f}, MAE={metrics_a['MAE']:.2f}, MAPE={metrics_a['MAPE']:.2f}%"
                )
                self.logger.log_narrative(
                    f"Comparison Metrics: RMSE={metrics_b['RMSE']:.2f}, MAE={metrics_b['MAE']:.2f}, MAPE={metrics_b['MAPE']:.2f}%"
                )
    ...
    def _write_markdown_output(self):
        ...
        # Write file
        output_path = self.output_dir / "sensitivity_results.md"
...
def parse_range(range_str: str) -> Tuple[int, int]:
    """Parse a range string like '100,150' into tuple (100, 150)."""
...
def run_sensitivity_analysis(
    output_dir: Optional[Path] = None,
    a_range: Optional[str] = None,
    b_range: Optional[str] = None,
):
    ...
    parsed_a_range = parse_range(a_range) if a_range else None
    parsed_b_range = parse_range(b_range) if b_range else None

    analysis = SensitivityAnalysis(output_dir, parsed_a_range, parsed_b_range)
